...
gulp.task('scripts:dist', function (foo) {
	...
	var merged = merge(
		...
		gulp.src([src.index, src.scripts], {cwd: src.cwd})
			...
			.pipe(ngmin())
			.pipe(concat(pkg.name + '.js', {process: function (src) {
...
var createModuleName = function (src) {
	return 'sn.control.' + src.split(path.sep)[0];
};
gulp.task('templates:dist', function () {
	...
	var merged = merge(
		...
		gulp.src(src.templates, {cwd: src.cwd})
			...
			.pipe(ngmin())
			.pipe(concat(pkg.name + '.tpl.js', {process: function (src) {
				return '// Source: ' + path.basename(this.path) + '\n' + (src.trim() + '\n').replace(/(^|\n)[ \t]*('use strict'|"use strict");?\s*/g, '$1');
			}}))
			.pipe(concat.header('(function(window, document, undefined) {\n\'use strict\';\n\n'))
			.pipe(concat.footer('\n\n})(window, document);\n'))
